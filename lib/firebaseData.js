...
export async function readUserProfile(uid) {
  ...
  return {
    uid,
    role: adminAllowed ? "admin" : profile.role || "",
    name: profile.name || "",
...
export async function saveUserProfile(uid, profile) {
  ...
  requireFirebase();
  const role = profile.role === "admin" ? "" : profile.role || "";
  ...
  const payload = {
    ...
    city: sanitizeText(profile.city || "Астана", 80),
...
export async function saveVendorProfile(vendorId, vendor) {
  ...
  const payload = {
    ...
    city: sanitizeText(vendor.city || "Астана", 80),
