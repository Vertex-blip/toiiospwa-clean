...
async function loadEventCollections(eventId) {
  if (!eventId) return {};
  const [guests, budget, checklist, tables, timeline, invitation] = await Promise.all([
...
async function loadFirebaseStore(user) {
  ...
  let next = {
    ...base,
    vendors: vendors.length ? vendors : base.vendors,
  ...
  const [profile, events, clientBookings, notifications] = await Promise.all([
  ...
  if (role === "vendor" || role === "admin") {
    ...
    const [allServices, allBookings, messages] = await Promise.all([
      role === "admin"
    ...
    next = {
      ...next,
      vendors: vendorRows.length ? vendorRows : next.vendors,
      services: role === "admin" ? allServices : allServices.filter((service) => vendorIds.has(service.vendorId)),
